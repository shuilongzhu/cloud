...
func DateNowFormatE(date time.Time) string {
	return date.Format(pojo.TimeLayout)
}
...
func TimestampToFormat(timeUnix int64) string {
	return time.Unix(timeUnix, 0).Format(pojo.TimeLayout)
}
...
func TimeStrToFormat(timeStr string) string {
	...
	return t.Format(pojo.TimeLayout)
}
...
func TimeStrToFormat2(timeStr string) string {
	t, err := time.ParseInLocation(pojo.TimeLayout, timeStr, time.Local)
	if nil != err {
